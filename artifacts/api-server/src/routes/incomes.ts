...
router.get("/incomes", async (req, res) => {
  ...
  const rows = await db.select().from(incomesTable).where(conditions.length ? and(...conditions) : undefined).orderBy(desc(incomesTable.date), desc(incomesTable.createdAt));
